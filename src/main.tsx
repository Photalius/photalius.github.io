...
(window as any)['Photalius'] = Photalius;
  ...
  ui: {
    ...
    nav: {
      items: [
        {
          name: 'stamp',
          icon: Photalius.defaultConfig.ui!.nav!.items!.find(
            i => i.name === 'frame'
          )!.icon,
          action: async function () {
            // photalius.tools.resize.apply({ width: 1080, height: 1080 });
            //photalius.tools.filter.apply('grayscale');
            // await (window as any).photalius.tools.import.addImageFromUrl(
